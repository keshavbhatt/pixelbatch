...
protected:
  void executeProcess(const QString &program, const QStringList &arguments, ImageTask *task) {
      QProcess *process = new QProcess(this);
      connect(process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this, [task, this, process](int exitCode, QProcess::ExitStatus exitStatus) {
          if (exitStatus == QProcess::NormalExit && exitCode == 0) {
              ...
              qWarning().noquote() << serializeProcessError(process, exitCode, exitStatus);
              emit optimizationError(*task, "Process failed with exit code: " + QString::number(exitCode));
          }
          process->deleteLater();
      });

      connect(process, &QProcess::errorOccurred, this, [task, this, process](QProcess::ProcessError error) {
          qWarning().noquote() << "Error occurred with the process:";
          qWarning().noquote() << serializeProcessError(process, 500, QProcess::CrashExit);
          emit optimizationError(*task, "ErrorString: " + process->errorString() + " ErrorCode: " + QString::number(error));
          process->deleteLater();
      });

      qDebug() << "Starting process for" << task->imagePath << "with program" << program << "and arguments" << arguments;
